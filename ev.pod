...
=item int ev_io_fd (const ev_io *)

Returns the file descriptor being watched. This accessor should be
preferred over directly reading the struct member, as libev stores
internal state in the watcher while it is inactive.
...
C<ev_io_set> for that. To obtain the descriptor value without exposing
libev’s internal bits, use C<ev_io_fd>.

=item int events [no-modify]

The set of events the fd is being watched for. Remember that this is a
bit set - to test for C<EV_READ>, use C<< w->events &
